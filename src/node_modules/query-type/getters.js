...
var debug = require('debug')('query-type/getters')
...
export const queryTypesByAgentId = createSelector(
  [queryTypesById, relationshipsByAgentId],
  (queryTypesById, relationshipsByAgentId) => {
    let result = {}

    each(relationshipsByAgentId, (relationships, agentId) => {
      // if (agentId === 'simon') {
      //   debug(agentId, relationships, queryTypesById)
      // }

      result[agentId] = reduce(relationships, (memo, relationship) => {
        let direction = findKey(relationship, v => v === agentId)
        //
        //
        // if (agentId === 'simon') {
        //   debug(relationship, direction)
        // }
        //
        let queryType = find(queryTypesById, {
          relationshipType: relationship.type,
          direction: direction
        })
        // if (agentId === 'simon') {
        //   debug('qt', queryType, direction)
        // }

        if (queryType && !memo[queryType.id]) {
          memo[queryType.id] = queryType
        }
...
export const agentQueryTypes = createSelector(
