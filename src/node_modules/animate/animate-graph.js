...
module.exports = function (options, next, done) {
  ...
  var interpolateZoom = InterpolateZoom(
    [options.oldVector[0], options.oldVector[1], options.oldScale || 1],
    [options.currentVector[0], options.currentVector[1], options.currentCcale || 1] //reset to default
  )
  ...
  function tick (timeStamp) {
    ...
    if (transform) {
      result.vector = [ transform[0], transform[1] ]
      result.scale = transform[2]
    }
    ...
    if (t0 < 1) {
      ...
      debug('finished transition')
        ...
        vector: options.currentVector || [0, 0],
        scale: options.currentScale || 1
      })
    }
  }
