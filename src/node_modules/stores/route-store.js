...
var routeStore = Reflux.createStore({
  ...
  onModelUpdate: function (payload) {
    if (payload.route.search === '') {
      this.queryMap = getDefaultQueryMap(payload.model)
    }
    else {
      this.queryMap = parseQuerytoMap(payload.route.query)
    }

    debug('queryMap', this.queryMap)
    this.queries = getQueries(payload.model, this.queryMap)
    this.trigger({ model: payload.model, queryMap: this.queryMap, queries: this.queries })
  },
  ...
  onRouteChange: function (payload) {
    ...
    if (payload.route.id !== this.modelId) {
      ...
      this.queries = getQueries(payload.model, this.queryMap)
      debug('queryMap', this.queryMap, this.queries)
