...
var routeStore = Reflux.createStore({
  ...
  onWheel: function (payload) {
    ...
    var transformed = matrixTransform(
      { vector: this.vector, scale: this.scale },
      { vector: payload.point, scale: dScale }
    )

    this.vector = transformed.vector
    this.scale = transformed.scale

    this.trigger({ vector: this.vector, scale: this.scale })
