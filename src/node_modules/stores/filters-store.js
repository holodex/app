...
var filtersStore = Reflux.createStore({
  ...
  onCancelTag: function (payload) {
    debug('onCanelTag', payload)
    if (payload.type === 'AgentType') {
      this.removeAgentTypeTag(payload)
    }
    else if (payload.type === 'Link' && payload.filter.linkType.id === subgroupLink) {
      var filtersByType = m.toJS(this.filtersByType)
      debug(filtersByType.targetAgent)
      var filter = find(filtersByType.targetAgent, function (filter) {
        return filter.id === payload.filter.id
      })
      debug('filter', filter)
      if (filter.agentType.active && filter.agentType.id === 'Group') {
        console.log('filtersStore', payload, filter)
        this.removeLink(payload)
  ...
  removeLink: function (payload) {
    debug('removeLink', payload)
    var filtersByType = m.toJS(this.filtersByType)
    var updatedFilters = filtersByType.targetAgent.map(function (filter) {
      if (filter.id === payload.filter.id) {
        var updatedFilter = clone(filter)
        debug('updated filter')
        updatedFilter.linkType.active = false
        return updatedFilter
      }
      else { return filter}
    })
