...
var getOptions2 = require('./get-options-2')
...
module.exports = React.createClass({
  ...
  render: function () {
    ...
    var activeQueries = parseQuery(this.props.route.query, this.props.model)
    var tags = getOptions2(this.props.model, activeQueries)
    debug('tags', tags, activeQueries)
    ...
    return r.div({ className: 'tags', style: this.style.index }, [
      ...
      r.div({ className: 'tags-filters', style: this.style.filters },
        tags.activeTags.map(function (tag, i) {
          var style = { order: tags.activeTags.length -1 - i }
          ...
          return r.div({ className: 'context-tag', style: this.style.contextTag }, [
            r.div(
              ...
              [r(this.props.Icon, { icon: tag.type === 'RoleType' ? 'circle-outline' : 'long-arrow-right' })]
            ),
            r(Tag, {
              color: tag.color,
              icon: cancelIcon,
              label: tag.label,
              trailingCancel: true,
              onIconClick: this.handleTagCancelClick({ filter: null, type: tag.type })
            })
          ])
        }.bind(this))
      ),
