...
module.exports = React.createClass({
  ...
  animateNodes: function (oldNodes, currentNodes, currentLinks) {
    ...
    function tick (timeStamp) {
      ...
      for (var i=0, len = transitionNodes.length; i < len; i++) {
        ...
        var node = transitionNodes[i]
        var y = interpolator[1](t1)
        var x = interpolator[0](t1)
        node.y = y
        node.x = x
        if (node.id === "http://localhost:3000/api/groups/purposive") debug('xy', x, y)
        if (interpolator[2]) {
  ...
  transitionToNode: function (model) {
    ...
    var nodesAndLinks = formatGraphData(model, tagData, this.state.centerCoords, options)


    cola.nodes(nodesAndLinks[0])
      .links(nodesAndLinks[1])
    cola.start(10, 15, 20) //compute new positions
    var nodes = ensurePostioning(model, cola.nodes(), this.state.centerCoords) //hack to acount for cola not always positioning fixed items

    this.animateNodes(this.state.nodes, nodes, cola.links()) //transition to new positions
  },
