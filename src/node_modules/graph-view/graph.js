...
module.exports = React.createClass({
  ...
  render: function () {
    ...
    var parents = this.props.model.parentGroups || []
    var parentNodes = parents.map(function (parent) {
      return {
        id: parent.getId(),
        model: parent,
        x: this.state.centerCoords[0], //NOTE assumes only 1 parent
        y: 0,
        w: 120,
        h: 120
      }
    }.bind(this))
    debug('model', this.state.nodes)
    ...
    return (
      r.div({ id: 'graph' }, [
        r.svg({
          ...
          r.defs(),
          r.g({ className: 'parents-group' }, [
            r.g({ className: 'parent-links' },
              parentNodes.map(function (parentNode) {
                return r.line({
                  key: parentNode.id + '-link',
                  x1: this.state.centerCoords[0], //NOTE assumes 1 parent
                  x2: this.state.centerCoords[0],
                  y1: 0,
                  y2: this.state.centerCoords[1],
                  stroke: vars.colors.gray,
                  strokeWidth: 2,
                  opacity: 0.5
                })
              }.bind(this))
            ),
            r.g({ className: 'parents-nodes'},
              parentNodes.map(function (parentNode) {
                return r(Node, {
                  node: parentNode,
                  key: parentNode.id,
                  handleClick: this.handleParentClick
                })
              }.bind(this))
            )
          ]
  ...
  componentDidMount: function () {
    ...
    var marker = h('marker',
      ...
      h('path', { d: 'M 0 0 L 10 5 L 0 10 z', fill: 'grey' }) //arrow marker
    )
  ...
  componentWillReceiveProps: function (nextProps) {
    ...
    // compute links
    var nodesAndLinks = formatGraphData(graph, tagData, model.getId(), this.state.centerCoords)
    ...
    if (nodes.length > 0) {
      ...
      cola.start(10, 15, 20)
        ...
        if (this.state.nodes) {
          debug('cola.nodes', cola.nodes())
          this.animateNodes(this.state.nodes, cola.nodes())
          this.setState({ cola: cola })
        }
        else {
          this.setState({ cola: cola, nodes: cola.nodes(), links: cola.links()  })
        }

    }
  },

  animateNodes: function (oldNodes, currentNodes) {
    ...
    for (var i=0, len = currentNodes.length; i < len; i++) {
      ...
      if (oldIdToIndex[currentNode.id]) { transitionNodes.push(currentNode) }
  ...
  transitionToParent: function (parentModel) {
    ...
    var parentNodesAndLinks = formatGraphData(parentModel.relationshipGraph, parentTagData, parentModel.getId(), this.state.centerCoords)
    debug('transitionToParent', parentTagData)
  },

  handleParentClick: function (model) {
    this.transitionToParent(model)
  }
