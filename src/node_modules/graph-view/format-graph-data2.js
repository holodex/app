...
module.exports = function (model, queryMap, queries, centerCoords, optionsMap) {
  ...
  debug('queryMap', queryMap, queries)
  ...
  debug('link', link)
    if (link.data.type.linkType && queryMap[link.data.type.linkType.id]) {
      ...
      if (queryMap[link.data.type.linkType.id].contexts
        ...
        query = find(queries.active, function (query) {
          debug('context queries', query)
          return query.id === link.data.type.linkType.id + '.contexts.' + link.data.relationship.context.id
        })
        ...
        query = find(queries.active, function (query) {
          debug('source queries', query)
          return query.id === link.data.type.linkType.id + '.sources.' + shortId(link.fromId)
        })
        ...
        query = find(queries.active, function (query) {
          debug('target queries', query)
          return query.id === link.data.type.linkType.id + '.targets.' + shortId(link.toId)
        })
  ...
  model.relationshipGraph.forEachNode(function (node) {
    if (typeof idToIndex[node.id] === 'number') {
      ...
      node.activeHalos = activeHalosMap[node.id]
