...
module.exports = React.createClass({
  getInitialState: function (options) {
    ...
    var prefix = Url.parse(config.api)
    var stewardType = prefix + '/relationshipTypes/steward';
  ...
  componentWillReceiveProps: function (nextProps) {
    ...
    var relationshipObj = this.state.relationshipObj;
    var nextPalleteMap = _.clone(this.state.palleteMap);
    var nextRelationshipObj = _.clone(relationshipObj);
   
    for (var i=0;i<relatedTypes.length;i++) { 
      //TODO only handles 7 colors with pale background
      var rel = relatedTypes[i];
      nextPalleteMap[rel] = pallete[i];
      nextRelationshipObj[rel] = (relationshipObj[rel]) ? true : false;
    }

    debug('palleteMap', relatedTypes, nextPalleteMap)
