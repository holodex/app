...
module.exports = React.createClass({
	...
	getInitialState: function (options) {
		...
		//compute links
		var makeLink = function (subj, relationship, obj) {
			return {
				source: subj.index,
				target: obj.index,
				model: relationship
		...
		for (var i=0;i<predicates.length;i++) {
			...
			if (subjectIds) {
				Object.keys(subjectIds).forEach(function (subjId) {
					var subject 	= _.find(nodes, function (node) { return node.values["@id"] === subjId; });
					var objectIds = edges[predicate][subjId];
					objectIds.forEach(function (objId) {
						var object 				= _.find(nodes, function (node) { return node.values["@id"] === objId; });
						var relationship 	= _.find(object.relationships.models, function (rel) {
							return (rel.values.is === subjId && rel.values.has === objId);
						});
