...
module.exports = function (model, filtersByType, centerCoords, options) {
    ...
    filters.some(function (filter) {
      if (filter.roleType.active) {
        if (filter.contextAgentId === model.getId()
          ...
          debug('active role in context', activeRole)
        }
        if (filter.roleType.active && node.data.rolesByType[filter.roleType.id]) {
          var activeRole = find(node.data.rolesByType[filter.roleType.id], function (role) {
            return role.pair.agent.getId() === filter.targetAgent.id
          })
          if (activeRole) {
            debug('active role', activeRole)
            ...
            idToIndex[node.id] = nodes.length -1
            debug('active role in context', node)

            return true
          }
        }
      }
      if (filter.linkType.active && filter.contextAgentId) {
        var activeLink = find(node.data.rolesByType[filter.roleType.id], function (role) {
          return role.linkTypeId === filter.linkType.id || role.pair.linkTypeId === filter.linkTypeId
        })
        if (activeLink) {
          debug('active link', activeLink)
          nodes.push(makeUINode(
            node,
            {
              size: vars.nodeSizes.default,
  ...
  graph.forEachLink(function (link) {
    ...
    var filterLink = filters.some(function (filter) {
    ...
    if (!filterLink && hiddenLink(link, idToIndex)) {
      links.push(makeUILink(
        link,
        {
          length: getLinkLength(activeLinks.length, 120),
