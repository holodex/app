...
function getLinkLength (activeLinks, l) {
  return Math.sqrt(activeLinks)*l
}
...
module.exports = function (graph, tagData, contextAgentId, centerCoords) {
  ...
  keys(triple).forEach(function (key) {
    ...
    if (expression.active) {
      ...
      if (expression.type === 'Role') activeRoleFilters.push(expression.id)
      if (expression.type === 'Link') {
        activeLinkFilters.push(expression.id)
        activeRelationships.push(expression.relationshipType)
      }
    }
  ...
  for (var i=0, len = filters.length; i<len; i++) {
    ...
    if (filter.active) {
      ...
      if (filter.type === 'Role') { activeRoleFilters.push(filter.id) }
        ...
        activeRelationships.push(filter.relationshipType)
      }
  ...
  graph.forEachNode(function (node) {
    ...
    var relMatches = intersection(activeRelationships, nodeRelationshipTypeIds) //link filters/expressions are 'greedy'
  ...
  graph.forEachLink(function (link) {
    ...
    var linkTypeId = sourceRole.type.getId() + '/link'
    var isActive = contains(activeLinkFilters, linkTypeId)
    var isHidden = ((hiddenMemberLink || !isActive) && linkTypeId === memberLinkTypeId)
    ...
    if ((isActive || isHidden) && sourceAndTargetPresent) {
      ...
      links.push({
        ...
        length: getLinkLength(activeLinkFilters.length+1,90),
      })
    }
  })
  debug('links')
