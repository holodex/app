...
function hiddenLink(link, idToIndex) {
  if (idToIndex[link.data.agent.getId()]) {
    var type = link.data.type.getId()
    if ( type === member || type === subgroup || type === group ) {
      return true
    }
    else { return false }
  }
...
module.exports = function (model, filtersByType, centerCoords, options) {
  ...
  for (var i=0, len = filters.length; i<len; i++) {
    ...
    if (filter.roleType.active && filter.contextAgentId) {
      activeRoles.push(filter.roleType.id)
      IdMap[filter.roleType.id] = filter
    }
    if (filter.linkType.active) activeLinks.push(filter.linkType.id)
  }

  debug('activeRoles', activeRoles)
  graph.forEachNode(function (node) {
    ...
    while (i > 0) {
      ...
      if (filter.contextAgentId === model.getId()
          && filter.roleType.active
          && node.data.rolesByType[filter.roleType.id]) {
        // debug('in context and node has role', filter, node)
        ...
        // })
        // if (activeRole) {
        debug('node has active role in context', filter, node)
        // IdMap[filter.roleType.id] = filter
        nodes.push(makeUINode(
          node,
          {
      ...
      if (filter.linkType.active) {
        debug('linkType active', filter, node)
  ...
  graph.forEachLink(function (link) {
    ...
    while (i > 0) {
      ...
      if (filter.linkType.active
        ...
        && (link.toId === filter.targetAgent.id || (link.data.relationship.context && link.data.relationship.context.getId() === filter.contextAgentId))) {
