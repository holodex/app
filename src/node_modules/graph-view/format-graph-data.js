  ...
  graph.forEachNode(function (node) {
    ...
    filters.some(function (filter) {
      if (filter.roleType.active) {
        if (filter.contextAgentId === model.getId() && node.data.rolesByType[filter.roleType.id]) {
          ...
          debug('active role in context', node.data)
        }
        if (filter.targetAgent.active && node.data.rolesByType[filter.roleType.id]) {
          var activeRole = find(node.data.rolesByType[filter.roleType.id], function (role) {
            return role.pair.agent.getId() === filter.targetAgent.id
          })
          if (activeRole) {
