...
function nodeInFilter(node, activeRoleFilters) {
  var match = getRoleMatches(node, activeRoleFilters)
...
module.exports = function (model, tagData, centerCoords) {
  ...
  keys(triple).forEach(function (key) {
    ...
    if (expression.active && expression.type === 'Link') activeRelationships.push(expression.relationshipType)
  ...
  graph.forEachNode(function (node) {
    ...
    if (isSubjectNode) {
      nodes.push(makeUINode(node, { size: vars.nodeSizes.default, activeRoleTypes: getRoleMatches(node, activeRoleFilters), IdMap: IdMap }))
      idToIndex[node.id] = nodes.length -1
      return
    }
    var isFilterNode = nodeInFilter(node, activeRoleFilters)
    if (isFilterNode) {
      nodes.push(makeUINode(node, { size: vars.nodeSizes.default, activeRoleTypes: getRoleMatches(node, activeRoleFilters), IdMap: IdMap }))
      idToIndex[node.id] = nodes.length -1
      return
