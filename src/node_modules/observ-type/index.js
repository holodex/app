...
var mapGraph = require('observ-graph/map')
...
function createType (defn) {
  ...
  // normalize type name(s) into array
  if (!Array.isArray(defn.type)) {
    defn.type = [defn.type]
  }
  ...
  function ObservType (context) {
    ...
    // setup type props
    //
    var setProps = mapValues(defn.props, resolve)
    var derivedProps = computedProps(setProps, defn.derived)
  ...
  // abstract function to initialize class with graph
  ObservType.context = function (graph) {
    initTypeGraph(ObservType, graph)
    //initViewGraphs(ObservType, ObservType.definition.views, graph)
  }
...
function initTypeGraph (Type, graph) {
  var plural = Type.definition.plural

  if (!graph[plural]) {
    graph[plural] = mapGraph(graph, function (entity) {
      console.log("types", Type.definition.type, entity.getType())
      if (any(entity.getType(), function (typeName) {
        console.log("type match?", typeName)
        return Type.definition.type.indexOf(typeName) !== -1
      })) {
...
function initViewGraphs (Type, viewsDefns, graph) {
