...
var debug = require('debug')('holodex:sheet-db')
...
module.exports = pollingGetGraph

function pollingGetGraph (cb) {
  getGraph(cb)

  return new CronJob({
    cronTime: config.data.cronSpec,
    onTick: function () { getGraph(cb) },
    start: true
  })
}
...
function getGraph (cb) {
  ...
  var graph = []
  var tasks = mapValues(worksheets, function (worksheet, collectionName) {
...
function trimify () {
  return through.obj(function (row, enc, cb) {
    row.value = mapValues(row.value, function (val) {
      return val.trim()
    })
    cb(null, row)
  })
