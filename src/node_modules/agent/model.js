    ...
    linkTypesByDirection: {
      ...
      fn: function () {
        ...
        keys(rolesByType).forEach(function (roleId) {
          ...
          if (role.type.linkType) linkTypesByDirection.asSource.push(role.type.linkType)
          if (pair.type.linkType) linkTypesByDirection.asTarget.push(pair.type.linkType)
        })
              ...
              if (role.agent && role.agent.getId() !== agentId) { //its the pair
                ...
                if (!filterMap[id]) {
                  var preDefault = {
                    ...
                    contextAgentId: null,
                    color: Labels[role.type.getId()]
                  }
                  var filter = setDefaultAgentFilters(preDefault, agentId, hasChildren)
                  memo.push(filter)
                  filterMap[id] = true
              ...
              ;[id0, id1].forEach(function (id, i) {
                if (!filterMap[id]) {
                  memo.push({
                    ...
                    color: Labels[relationship.roles[i].type.getId()]
                  })
                  filterMap[id] = true
                }
              })
