...
module.exports = Model.extend({
  ...
  derived: {
    ...
    filters: {
      cache: false,
      deps: ['roles', 'relationships'],
      fn: function () {
        var agentId = this.getId()
        var filterMap = {}
        return this.roles.reduce(function (memo, role) {
          var id = role.pair.agent['@type']  + '-' + role.pair.type.getId()
          if (!filterMap[id]) {
            memo.push({
              id: id,
              agentType: role.pair.agent['@type'],
              roleTypeId: role.pair.type.getId(),
              linkTypeId: role.linkTypeId,
              agentId: role.agent.getId()
            })
            filterMap[id] = true
          }
          return memo
            ...
            .reduce(function (memo, relationship) {
              ...
              ;[id0, id1].forEach(function (id, i) {
                if (!filterMap[id]) {
                  memo.push({
                    id: id,
                    agentType: relationship.roles[i].agent['@type'],
                    roleTypeId: relationship.roles[i].type.getId(),
                    linkTypeId: relationship.roles[i].linkTypeId,
