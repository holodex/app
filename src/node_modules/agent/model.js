  ...
  derived: {
    ...
    filtersByType: {
      ...
      fn: function () {
        ...
        var filterMap = {}
        var filtersByType = {}
        filtersByType.agent = this.roles.reduce(function (memo, role) {
          var id = role.pair.agent['@type']  + '-' + role.pair.type.getId()
          if (!filterMap[id]) {
            memo.push({
              id: id,
              agentType: { id: role.pair.agent['@type'], active: false },
              roleType: { id: role.pair.type.getId(), active: false },
              linkType: { id: role.pair.linkTypeId, active: false },
              agent: { id: role.agent.getId(), active: false }
            })
            filterMap[id] = true
          }
          return memo
